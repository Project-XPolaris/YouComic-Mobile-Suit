...
class DetailProvider with ChangeNotifier {
  ...
  loadRelateArtist() async {
    if (book.tags.lastIndexWhere((tag) => tag.type == "artist") != -1) {
      ...
      relateArtistBookDataSource.books.removeWhere((book) => book.id == this.book.id);
      notifyListeners();
    }
  }
  ...
  loadRelateSeries() async {
    if (book.tags.lastIndexWhere((tag) => tag.type == "series") != -1) {
      ...
      relateSeriesBookDataSource.books.removeWhere((book) => book.id == this.book.id);
      notifyListeners();
    }
  }
  ...
  loadRelateTheme() async {
    if (book.tags.lastIndexWhere((tag) => tag.type == "theme") != -1) {
      ...
      relateThemeBookDataSource.books.removeWhere((book) => book.id == this.book.id);
      notifyListeners();
    }
  }
  ...
  loadCollections() async {
    ...
    collectionDataProvider.loadCollections(true);
    notifyListeners();
  }
